...
** All Rights Reserved
**
** Started: 25 February 1991
**
** Revision Information: $Logfile:   G:/simplex/smp/vcs/s_file.c_v  $
**                       $Date:   25 Oct 1992 14:26:36  $
**                       $Revision:   1.8  $
**
*/
...
int pascal get_firstdir(struct fi *fi,char *mask)
	{
	int rtn;
	int found = 0;
	...
	search = 1;
	hdir = HDIR_CREATE;
	rtn = (int)DosFindFirst(mask,&hdir,FILE_DIRECTORY,&find,sizeof(FILEFINDBUF),&search,0L);
	if (!rtn)
		{
		strcpy(fi->fi_name,find.achName);
		strupr(fi->fi_name);
...
#else
	rtn = (int)_dos_findfirst(mask,_A_SUBDIR,&find);
	if (!rtn)
		{
		strcpy(fi->fi_name,find.name);
		strupr(fi->fi_name);
...
int pascal get_nextdir(struct fi *fi)
	{
	int rtn;
	int found = 0;
